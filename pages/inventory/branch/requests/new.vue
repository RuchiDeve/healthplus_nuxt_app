        ...
        <div class="col-4">
          <div class="row">
            <div class="col-12">
              <div class="card">
                <div class="card-body">
                  <vs-select block v-if="productRequest.assignedBranches.length"
                             placeholder="Select an assigned branch..."
                             v-model="productRequest.payload.branch_id">
                    <vs-option
                    ...
                    <vs-option
                      :key="index"
                      :label="branch.name"
                      v-for="(branch, index) in productRequest.assignedBranches"
                      :value="branch.id">
                      {{ branch.name }}
                    </vs-option>
                  </vs-select>
...
export default {
  ...
  methods: {
    async placeNewRequest() {
      ...
      try {
        let requestResponse = await this.$axios.$post("/inventory/branch/request/save", this.productRequest.payload);

        console.log('product request response', requestResponse);

        await this.$router.push("/inventory/branch/requests");
  ...
  computed: {
    ...
    selectedBranch(){
      if (this.productRequest.payload.branch_id === -1) return null;
      return this.productRequest.assignedBranches.find((branch) => branch.id === this.productRequest.payload.branch_id);
    }
  },
  mounted() {
  ...
  watch: {
    'productRequest.payload.branch_id': function(branch_id, __){
      if(branch_id){
        this.fetchProductsInCountry(this.selectedBranch.country_id);
      }
    }
  }
}
