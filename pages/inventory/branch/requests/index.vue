    ...
    <div class="col-12">
      <div class="row mb-3">
        <div class="col-12 d-flex justify-content-between">
          ...
          <vs-button active success square flat
                     :href="'/inventory/branch/requests/new'"
                     v-if="$root.authUser && $root.authUser.is_branch_admin">
      ...
      <div class="row">
          ...
          <div class="card" >
            <div class="card-body d-flex justify-content-between align-items-center">
              <div class="d-flex align-self-center" v-if="requests.totalPages">
                <vs-pagination only-arrows v-model="requests.page" :length="requests.totalPages" />
            ...
            <div class="table-responsive">
              <table class="table table-hover table-condensed" style="table-layout: initial">
                ...
                <tr style="cursor: pointer"
                  ...
                  <td>{{index + 1}}</td>
                  <td>
                    <span class="font-weight-semibold">{{request.branch.name}}</span>
                  </td>
                  <td>
                  ...
                  <td>
                    <div v-if="!!request.issued_by">
                      by: <span class="font-weight-semibold">{{request.issued_by_user.username}}</span>
                      <br>
                  ...
                  <td>
                    <div class="dropdown">
                      ...
                      <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                        ...
                        <a v-if="(!!!request.issued_by) && !$root.authUser.is_branch_admin" class="dropdown-item" href="javascript:void(0)"
                           @click="issueRequest(request)">
                          <i class="bx bxs-check-circle"></i>
                          Send Products
                        </a>

                        <a v-if="(request.issued_by !== null) && !request.received && $root.authUser.is_branch_admin" class="dropdown-item" href="javascript:void(0)"
                           @click="receiveRequest(request)">
                          <i class="bx bxs-check-circle"></i>
                          Receive Products
                        </a>
        ...
        <div class="col-12 mt-4" v-if="requests.totalPages">
          <vs-pagination v-model="requests.page" :length="requests.totalPages" />
        </div>
      </div>
    </div>

    <vs-dialog blur v-model="requestProductModal.show" @close="requestProductModal.show = false">
      <template #header>
        <h4 class="not-margin" v-if="requestProductModal.selectedRequest">
          {{requestProductModal.selectedRequest.branch.name}}:
          <b>Items Requested</b>
        </h4>
      </template>
      ...
      <div class="row" v-if="requestProductModal.selectedRequest">
        <div class="col-12">
          <div class="pt-0 table-responsive" v-if="requestProductModal.selectedRequest.items.length">
            <table class="table table-hover table-condensed" style="table-layout: initial">
              ...
              <tbody>
              <tr v-for="(cartItem, index) in requestProductModal.selectedRequest.items" :key="index">
  ...
  methods: {
    ...
    async issueRequest(request){
      ...
      await this.$doTask(async ()=>{
        let response = await this.$axios.$post('inventory/branch/request/issue', {
          'request_id': request.id,
        });

        window.location.reload();

      }, 'Issuing request', this.$refs['requests-list']);
    ...
    async receiveRequest(request){
      ...
      await this.$doTask(async ()=>{
        let response = await this.$axios.$post('inventory/branch/request/receive', {
          'request_id': request.id,
        });

        window.location.reload();

      }, 'Receiving request', this.$refs['requests-list']);
    ...
    getProductRequests: _.debounce(async function () {
      await this.$doTask(
        async () => {
          this.setSearch()
          let response = await this.$axios.$get("inventory/branch/requests", {
            params: {
